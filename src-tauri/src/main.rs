...
use learningtauri_lib::{ext_mod, tauri_generate_context};
...
fn main() -> Result<Infallible, Box<dyn Error>> {
    ...
    // 👉 Equivalent to `python -m learningTauri`,
    // i.e, run the `src-tauri/python/learningTauri/__main__.py`
    let py_script = PythonScript::Module("learningTauri".into());
